/* Source File Name: GreenButtonController.cs
...
public class GreenButtonController : MonoBehaviour {
	...
	// When the player is within the trigger, if the player has code2, green lock is 
	// unlocked, otherwise lock up all locks
	void OnTriggerStay(Collider other){
		if (other.CompareTag ("Player")) {
			if (CrossPlatformInputManager.GetButtonDown("Fire1")){
				if(gameController._HasCode2){
					this.clips[0].Play();	
					gameController.UnlockGreenLock();
					this.gameController.infoLabel.text = "Green Exit Lock is Lifted";
